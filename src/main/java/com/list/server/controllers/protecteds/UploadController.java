...
public class UploadController {
    ...
    public ResponseEntity<String> uploadImage(@PathVariable("type") String type, @RequestParam("file")MultipartFile file) {
        try {
            String fileName = service.store(file, type);
            return ResponseEntity.ok("File uploaded successfully: " + fileName);
        } catch (UploadException exception) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to store file: " + exception.getMessage());
        }
    }
